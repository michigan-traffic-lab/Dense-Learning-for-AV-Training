...
class Simulator(object):    
    ...
    def __init__(self,
        ...
        output=None,
        replay_flag=False,
        config={"max_obs_range": 115}, worker_id=None):
        """Initialize the Simulator class.

        Args:
            sumo_config_file_path (str): Path of the sumo configuration file.
            sumo_net_file_path (str): Path of the sumo network file.
            sumo_control_state (bool, optional): Whether to control the simulation. Defaults to True.
            pre_step (function, optional): Pre-step function. Defaults to dummy_function.
            num_tries (int, optional): Number of tries. Defaults to 10.
            step_size (float, optional): Step size in seconds. Defaults to 0.1.
            lc_duration (int, optional): Lane change duration. Defaults to 1.
            action_step_size (float, optional): Action step size. Defaults to 0.1.
            sublane_flag (bool, optional): Whether to use sublane. Defaults to True.
            gui_flag (bool, optional): Whether to use GUI. Defaults to False.
            track_cav (bool, optional): Whether to track CAV. Defaults to False.
            input (dict, optional): Input. Defaults to None.
            input_path (str, optional): Input path. Defaults to None.
            experiment_path (str, optional): Experiment path. Defaults to None.
            output (list, optional): Output. Defaults to None.
            replay_flag (bool, optional): Whether to replay the recorded trajectories. Defaults to False.
            config (dict, optional): Configuration. Defaults to {"max_obs_range": 115}.
        ...
        self.replay_CAV_flag = True
        self.input_trajs = None
        self.sumo_time_stamp = 0.0
        if replay_flag:
            trajs_exist_flag = os.path.isdir(input_path)
            if trajs_exist_flag:
                input_file_list = glob.glob(input_path+"/crash/*.fcd.xml")
                self.input_file_dict = {}
                for file in input_file_list:
                    file_name = file.split("/")[-1]
                    episode_num = int(file_name.replace(".fcd.xml",""))
                    json_file = file.replace("fcd.xml","json")
                    self.input_file_dict[episode_num] = [file,json_file]
            else:
                raise ValueError("Input path does not exist, please check!")
        ...
        if self.sumo_control_state:
            ...
            if self.output is not None:
                ...
                if "fcd" in self.output:
                    ...
                    fcd_output_path = os.path.join(self.experiment_path, "crash", fcd_output_filename)
                    sumoCmd += ["--fcd-output", fcd_output_path, "--fcd-output.acceleration", "--device.fcd.explicit", "CAV", "--device.fcd.radius", "150"]
    ...
    def step(self, drl_action=None):
        ...
        Args:
            drl_action (int, optional): Action from DRL agent. Defaults to None.
        """        
        self.pre_step(self)
        self.env.step(action=drl_action)
        if self.sumo_control_state:
            sim_step = traci.simulation.getTime()+self.action_step_size
            self.sumo_time_stamp = self.get_time()
            if self.replay_flag or conf.train_mode == "offline":
                self.env._replay_vehicle_list()
            traci.simulationStep(step=sim_step)
            if self.replay_flag or conf.train_mode == "offline":
                sumoT = round(self.get_time()-0.1,1)
                # if sumoT <= round(float(self.env.init_time_step),1):
                #     self.env.vehicle_list["CAV"].lateral_speed = utils.remap(self.env.replay_json_obj["CAV_info"][str(sumoT)]["CAV_action"]["additional_info"][0][1], [-1, 1], [-conf.lat_v_max, conf.lat_v_max])
    ...
    def soft_run(self):
        ...
        """
        if self.replay_flag:
            self.input_trajs = utils.load_trajs_from_fcdfile(self.input_file_dict[self.episode][0])
            try:
                with open(self.input_file_dict[self.episode][1]) as f:
                    self.input_trajs_add = ujson.load(f)
            except:
                self.input_trajs_add = None
        self.env.initialize()
        self.sumo_time_stamp = self.get_time()
    ...
    def plain_traci_step(self):
        """Run the simulation for one step for SUMO.
        """
        traci.simulationStep()
    ...
    def run(self, episode):
        ...
        if not self.replay_flag or (self.replay_flag and episode in self.input_file_dict.keys()):
            self.start()
            self.soft_run()
            self.stop()
    ...
    def stop(self):
        ...
        if self.started:
            ...
            self.started = False
            if os.environ["mode"] == "testing":
                self.env.info_extractor.handle_fcd_files()
    ...
    def set_vehicle_emegency_deceleration(self, vehID, decel):
        """Set emergency deceleration for the vehicle.

        Args:
            vehID (str): Vehicle ID.
            decel (float): Emergency deceleration in m/s^2.
        """
    ...
    def get_vehicle_distance_to_edge(self, veh_id, edge_id, edge_position):
        """Get the distance between the vehicle and the edge.

        Args:
            veh_id (str): Vehicle ID.
            edge_id (str): Edge ID.
            edge_position (float): Position on the edge.

        Returns:
            float: Distance between the vehicle and the edge.
        """
    ...
    def get_ego_vehicle(self, vehID, dist = 0.0):
        ...
        if dist <= self.config["max_obs_range"]:
            ...
            try:
                ...
                ego_info = utils.check_vehicle_info(vehID, ego_info, self)
    ...
    def get_vehicle_heading(self, vehID):
        ...
        Returns:
            float: Vehicle heading in degree.
        """
    ...
    def set_gui_schema(self, schema_name):
        """Set the color scheme of the GUI.

        Args:
            schema_name (str): Scheme name.
        """
        traci.gui.setSchema(viewID="View #0", schemeName=schema_name)
    ...
    def set_vehicle_speed(self, vehID, speed):
    ...
    def _cal_lateral_distance(self, vehID, direction):
        ...
        lat_offset = traci.vehicle.getLateralLanePosition(vehID)
        if direction == "left":
            ...
            latdist = (origin_lane_width+target_lane_width)/2-lat_offset
    ...
    def get_vehicle_speed_without_traci(self, vehID):
