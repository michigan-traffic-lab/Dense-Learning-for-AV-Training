...
def sample_eval_index_list_eval_worker(data_info_new):
    ...
    if isinstance(data_info_new, list):
        ...
        if conf.train_case_study_mode == "crash_nearmiss":
            ...
            print(safe_traj_num, safe_traj_weight1_sample_num, safe_traj_weight0_sample_num)
            print(len(safe_traj_weight1_index_list[0]), len(safe_traj_weight0_index_list[0]))
            ...
            if safe_traj_weight0_sample_num > 0:
                ...
                eval_index_list0 = np.array([]).astype(int)
            print(
...
print(ray.nodes())
if conf.experiment_config["ablation_study_config"] in ["NEDD","NRDD", "NNME_NRDD"]:
    tune.run(
        "PPO",
